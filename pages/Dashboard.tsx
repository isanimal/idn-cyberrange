...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const nextModule = MODULES.find(m => m.progress === 0 && !m.isLocked);
        ...
        <Card className="bg-white dark:bg-gradient-to-br dark:from-slate-800 dark:to-slate-900 border-slate-200 dark:border-transparent">
          <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-slate-800 dark:text-white mt-1">{user.points}</h3>
            </div>
        ...
        <Card>
           <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-slate-800 dark:text-white mt-1">{user.completedModules} <span className="text-sm text-slate-400 font-normal">/ {MODULES.length}</span></h3>
            </div>
        ...
        <Card>
           <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-slate-800 dark:text-white mt-1">0</h3>
            </div>
        ...
        <Card>
           <div className="flex items-start justify-between">
            <div>
              ...
              <h3 className="text-3xl font-bold text-slate-800 dark:text-white mt-1">#42</h3>
            </div>
          ...
          {activeModules.length > 0 ? activeModules.map(module => (
            ...
            <Card className="hover:border-idn-500 dark:hover:border-idn-500 transition-colors group cursor-pointer">
              <div className="flex justify-between items-center mb-4">
                 ...
                 <span className="px-2 py-1 bg-slate-100 dark:bg-slate-900 rounded text-xs font-mono text-slate-500">{module.level}</span>
              </div>
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-4">{module.description}</p>
              <div className="w-full bg-slate-100 dark:bg-slate-900 rounded-full h-2 mb-2">
                <div className="bg-idn-500 h-2 rounded-full" style={{ width: `${module.progress}%` }}></div>
              </div>
              <div className="text-right text-xs text-idn-600 dark:text-idn-400 font-bold">{module.progress}% Complete</div>
            </Card>
            </Link>
          )) : (
            <Card className="border-dashed border-2 border-slate-300 dark:border-slate-700 bg-transparent shadow-none">
              <div className="text-center py-8">
                <p className="text-slate-400">No active modules. Start a new one from the catalog!</p>
              </div>
            </Card>
          )}
                ...
                <Card className="hover:border-idn-500 transition-colors group cursor-pointer opacity-90 hover:opacity-100">
                  <div className="flex justify-between items-center">
                    <div>
                      ...
                      <p className="text-sm text-slate-500 dark:text-slate-400">{nextModule.description}</p>
                    </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <ul className="space-y-4">
              {[1, 2, 3].map((i) => (
                <li key={i} className="flex gap-3 pb-4 border-b border-slate-100 dark:border-slate-700 last:border-0 last:pb-0">
                  <div className="w-2 h-2 rounded-full bg-idn-500 mt-2"></div>
                  <div>
                    <p className="text-sm text-slate-600 dark:text-slate-300">Submitted flag for <span className="text-idn-600 dark:text-idn-400 font-medium">Challenge #{i}</span></p>
